...
class BundleBuilderApi {
    ...
    constructor() {
        ...
        try {
            ...
            KalturaLogger.error("EXITING: " + BundleBuilderServerErrors.ASYNC_PROXY_SERVER_INIT_ERROR + ": " + util.inspect(err));
            process.exit(1);
    ...
    _startServer() {
        ...
        KalturaLogger.log('Bundle-Builder-Server ' + this._version + ' started');
    ...
    _configureBundnleBuilderServer(app) {
        ...
        app.get('/build',
            function (request, response) {
                ...
                KalturaLogger.log("Bundle id: " + bundlerName + "\ndest: " + dest + "\nconfig: " + bunderConfig);
                var child = exec('gulp build --silent --gulpfile ' + gulpfilePath + ' --name ' + bundlerName + ' --config "' + bunderConfig + '"' + ' --dest ' + dest + ' --source ' + bundleSource, {maxBuffer: 1024 * 1000000000}, function (err, stdout, stderr) {
                    ...
                    if (err) {
                        KalturaLogger.log("Bundle id: " + bundlerName + " - build error!!!\nerr: " + err + "\nelapsed time: " + elapsedTime + "ms");
                        response.status(500).end(err);
                    } else {
                        KalturaLogger.log("Bundle id: " + bundlerName + " - build success!!!\nelapsed time: " + elapsedTime + "ms");
