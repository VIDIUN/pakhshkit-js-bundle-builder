...
RequestUtils = {
    ...
    getVidiunSignedIp: function(request) {
        let vidiunSignedIp = null;

        let vidiunRemoteAddrHeader = request.headers['X-VIDIUN-REMOTE-ADDR'] || request.headers['x-vidiun-remote-addr'];
        let salt = config.get('server.remote_addr_header_salt');
        let timeout = config.get('server.remote_addr_header_timeout');

        if(!vidiunRemoteAddrHeader || !salt || !timeout)
            return vidiunSignedIp;

        let vidiunRemoteAddrHeaderParts = vidiunRemoteAddrHeader.split(",");
        let signedIp = vidiunRemoteAddrHeaderParts[0];
        let time = vidiunRemoteAddrHeaderParts[1];
        let uniqueId = vidiunRemoteAddrHeaderParts[2];
        let hash = vidiunRemoteAddrHeaderParts[3];
    ...
    getRemoteIpAddress: function(request) {
        ...
        let remoteAddress = this.getVidiunSignedIp(request);
        if(remoteAddress)
            return remoteAddress;
